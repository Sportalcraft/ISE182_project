...
{
    [Serializable]
    // This class represent a messgae in the chatroom with the properties and methods that can be publish to the users 
    public class DisplayMessage : IMessage
    {
        ...
        public DisplayMessage(Guid guid, DateTime receivingTime, string userName, int GroupID, string body)
        {
        ...
        //A copy constractor
        public DisplayMessage(IMessage msg) : this(msg.Id, msg.Date, msg.UserName, int.Parse(msg.GroupID), msg.MessageContent)
        {
            Logger.Log.Debug(Logger.MethodStart(MethodBase.GetCurrentMethod()));
        }

        //A constractor of message class
        public DisplayMessage(Guid guid, DateTime receivingTime, int group, string nickName, string body) : this(guid, receivingTime, nickName, group, body)
        {
            Logger.Log.Debug(Logger.MethodStart(MethodBase.GetCurrentMethod()));
        }

        //A constractor of message class
        public DisplayMessage(int group, string nickName, string body) : this(Guid.NewGuid(), DateTime.Now.ToUniversalTime(), nickName, group, body)
        {
            Logger.Log.Debug(Logger.MethodStart(MethodBase.GetCurrentMethod()));
        }
